...
class WebService: WebServiceProtocol {
    ...
    func download(fromURL: String, headers: [HeaderElement] = []) async throws -> URL {
        ...
        guard let response = response as? HTTPURLResponse else { throw NetworkError.badResponse }
        guard response.statusCode >= 200 && response.statusCode < 300 else {
            throw NetworkError.badStatus(code: response.statusCode, data: localUrl.dataRepresentation)
        }
        return localUrl
    }
    ...
    func postMultiPartFormData<T: Codable>(
        ...
        for element in data {
            ...
            body.append(Data("Content-Disposition: form-data; name=\"\(element.name)\"".utf8))
            if let filename = element.filename {
                body.append(Data("; filename=\"\(filename)\"".utf8))
            }
            body.append(Data("\r\n".utf8))
    ...
    var errorDescription: String? {
        ...
        case .badStatus(let code, _):
            return String(format: NSLocalizedString(
                "Received an unexpected HTTP status code: %d.",
                comment: "Unexpected Status Code"), code)
        case .failedToDecodeResponse:
